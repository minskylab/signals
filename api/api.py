...
extractor_thread = threading.Thread(target=principal.launch)
...
def get_csv(filename: str):
    ...
    print(f"total tweets={len(tweets)} | time={end - start}")

    if len(tweets) == 0:
        return "I don't have tweets in this date range, try with other", 404
